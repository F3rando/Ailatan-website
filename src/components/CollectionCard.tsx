...
export function CollectionCard({
  ...
  imageClassName = 'w-full h-full object-cover grayscale brightness-50 transition-[transform,filter] duration-[0.8s] ease-slow scale-105 group-hover:scale-110 group-hover:grayscale-[40%] group-hover:brightness-75 group-hover:contrast-125',
}: CollectionCardProps) {
  return (
    <Link to={to} className={`group relative bg-surface overflow-hidden cursor-pointer text-left w-full ${aspectClassName}`}>
      <div className="absolute inset-0 w-full h-full overflow-hidden bg-black">
        <img src={imageSrc} alt={imageAlt} className={imageClassName} loading="lazy" />
      </div>
